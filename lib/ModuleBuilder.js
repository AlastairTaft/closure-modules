...
var Builder = function(jsSources){
  ...
  // These will get deleted afterwards
  //this.tempFiles = []

  this.compilationLevel = 'ADVANCED'
}
...
Builder.prototype.addModule = function(scriptPath, opt_outputName){
  ...
  // We need to create a temp file that includes the goog.require('loadModule')
  /*var tempName = null
    , dir = path.dirname(scriptPath)
    , ext = path.extname(scriptPath)
    , basename = path.basename(scriptPath, ext)
  do {
    tempName = dir + '/' + basename + '-'  
      + Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 5)
      + ext
  } while (fs.existsSync(tempName));

  fs.writeFileSync(tempName,  
    fs.readFileSync(scriptPath, {encoding: 'utf8'})
    + '\ngoog.require(\'loadModule\');'
  )*/

  //this.tempFiles.push(tempName)
  //this.modules.push([tempName, entryPoint, opt_outputName || entryPoint])
...
*/
  ...
  ccp.stderr.on('data', function (data) {
    errStr += data
    if (typeof self.suppressWarnings === 'undefined' 
  ...
  ccp.on('close', function (code) {
    ...
    //self.tempFiles.forEach(function(t){
    //  fs.unlinkSync(t)
    //})
...
Builder.prototype.suppressWarnings = function(value){
  this.suppressWarnings = value
}

Builder.prototype.setCompilationLevel = function(value){
  this.compilationLevel = value
}
